...
const DeveloperDetails = () => {
  ...
  const fetchDeveloperData = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/users/developer_details/${id}`);
  ...
  return (
    <div>
      <Descriptions title="Developer Details">
        ...
        <Descriptions.Item label="Email">{developerData.email}</Descriptions.Item>
        <Descriptions.Item label="Level">{developerData.level}</Descriptions.Item>
